...
?>
  ...
  <section class="sub-bnr" data-stellar-background-ratio="0.5">
    <div class="position-center-center">
      <div class="container">
        <h4><?php echo $cat_n ?><?php echo $sub_cat_n ?> PRODUCTS</h4>
  ...
  <div id="content"> 
    ...
    <!-- Popular Products -->
    <section class="shop-page padding-top-20 padding-bottom-50">
      <div class="container">
...
$ss=$_POST['sort'];
   ...
   ?>
                ...
                <form action="" method="POST">
                <!-- Short By -->
                <select class="selectpicker" name="sort" onchange="form.submit();" >
<option selected="true" disabled="disabled">Sort By</option>  
                  <option value="latest" <?php if($ss=="latest"){echo "selected='selected'"; } ?>  >Latest</option>
                  <option  value="high"  <?php if($ss=="high"){echo "selected='selected'"; } ?> >Price - High to Low</option>
                  <option value="low"  <?php if($ss=="low"){echo "selected='selected'"; } ?> >Price - Low to High</option>
                </select>
                <!-- Filter By -->
                     </form>  
               </div>
...
if ($ss=="latest") {
    $res=mysqli_query($con,"select * from products  order by d_o_c desc");
}
else if($ss=="high"){
      $res=mysqli_query($con,"select * from products  order by price asc");

}
else if($ss=="low"){
      $res=mysqli_query($con,"select * from products  order by price desc");
...
{
  $see=$_GET['search'];
      $res=mysqli_query($con,"select * from products where search_tags LIKE '%{$see}%'");

}
if (isset($_GET['cat'])) {
  if (empty($ss)) {
    $res=mysqli_query($con,"select * from products where cat='$cat_n'");
  }
  else{

if ($ss=="latest") {
    $res=mysqli_query($con,"select * from products where cat='$cat_n' order by d_o_c desc");
}
else if($ss=="high"){
      $res=mysqli_query($con,"select * from products where cat='$cat_n' order by price desc");

}
else if($ss=="low"){
      $res=mysqli_query($con,"select * from products where cat='$cat_n' order by price asc");
...
if (isset($_GET['subcat'])) {
    ...
    $res=mysqli_query($con,"select * from products where sub_cat='$sub_cat_n'");
}
else{

if ($ss=="latest") {
    $res=mysqli_query($con,"select * from products where sub_cat='$sub_cat_n' order by d_o_c desc");
}
else if($ss=="high"){
      $res=mysqli_query($con,"select * from products where sub_cat='$sub_cat_n' order by price desc");

}
else if($ss=="low"){
      $res=mysqli_query($con,"select * from products where sub_cat='$sub_cat_n' order by price asc");
                              ...
                              { 
                                $disa=$dis_text="";
if($row['out_of_stock']=="on"){   $disa="disabled";  $dis_text="<br><h6>OUT OF STOCK</h6>";  }

echo '                        
          <!-- Item -->
          <div class="col-md-3 '.$disa.'">
            <div class="item">            
            <!-- Sale Tags -->
            <!-- <div class="on-sale">
            10%
        ...
        <div class="item-img"> <img class="img-1" src="admin/product_images/'.$row['img_1'].'" alt="" > 
              </div>
              <!-- Item Name -->
              <div class="item-name"> '.$row['name'].''.$dis_text.'</a>
                <p>'.$row['short_d'].'</p>
